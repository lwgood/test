...
3. [K8S 组件组成](./README.md/#k8s-组件组成)
4. [K8S 不同应用 DaemonSet、Deployment、StatefulSet 和 ReplicaSet 的区别](./README.md/#k8s-不同应用-daemonsetdeploymentstatefulset-和-replicaset-的区别)
5. [K8S 流量的治理](./README.md/#k8s-流量的治理)
6. [502 Bad Gateway 错误分析](./README.md/#502-bad-gateway-错误分析)
7. [504 Gateway Timeout 错误分析](./README.md/#504-gateway-timeout-错误分析)
8. [Redis 的四种模式分析对比](./README.md/#redis-的四种模式分析对比)
...
| **使用场景** | - HTTP/HTTPS（网页浏览）<br>- FTP（文件传输）<br>- 数据库连接（数据一致性） | - 视频直播（实时性要求高）<br>- 在线游戏（实时互动）<br>- DNS 查询（简短请求/响应） |
